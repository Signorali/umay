...
class BackupService:
    ...
    async def restore_backup(self, filename: str, actor_id: uuid.UUID) -> dict:
        ...
        # Step 3: Apply any new migrations on top of restored schema
        import subprocess as _sp
        _sp.run(["alembic", "upgrade", "head"], capture_output=True, cwd="/app")

        # Step 4: Restart the backend container so connection pool refreshes.
        # Detached subprocess — response is returned before the restart happens.
        # Try both Docker Compose v2 (umay-backend-1) and v1 (umay_backend) naming.
        _sp.Popen(
            "sleep 5 && (docker restart umay-backend-1 2>/dev/null || docker restart umay_backend 2>/dev/null)",
            shell=True,
            stdout=_sp.DEVNULL,
            stderr=_sp.DEVNULL,
        )
