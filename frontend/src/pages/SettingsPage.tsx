...
import {
  ProfileIcon, SecurityIcon, AppearanceIcon, ExportIcon, LinkIcon,
  LicenseIcon, SettingsIcon, UsersIcon, GroupsIcon, RolesIcon,
  DeleteIcon, AuditIcon, BackupIcon, SearchIcon, RefreshIcon, MoonIcon,
  ...
  const NAV_GROUPS = [
    {
      ...
      items: [
        ...
        { id: 'security', label: 'Şifre & Güvenlik' },
    ...
    ...(isAdmin ? [{
      label: 'Uyum',
      items: [
        { id: 'delete-requests', label: 'Silme İstekleri' },
    ...
    <div>
      ...
      <div className="page-header" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          <div className="avatar avatar-lg" style={{ background: 'var(--accent-subtle)', color: 'var(--accent)', fontSize: 20 }}>
            {user?.full_name?.slice(0, 2).toUpperCase()}
          </div>
          <div>
            <h1 className="page-title" style={{ marginBottom: 2 }}>{user?.full_name}</h1>
            <p className="page-subtitle" style={{ marginBottom: 0 }}>
              {user?.email}
              {user?.is_superuser && (
                <span className="badge badge-accent" style={{ marginLeft: 8 }}>Süper Kullanıcı</span>
              )}
            </p>
          </div>
        </div>
              ...
              {group.items.map(item => (
                <button
                  ...
                  style={{
                    ...
                    background: tab === item.id ? 'var(--accent-subtle)' : 'transparent',
                    transition: 'all 0.15s',
                    textAlign: 'left',
                  }}
                  onMouseEnter={e => {
                    if (tab !== item.id) (e.currentTarget as HTMLButtonElement).style.background = 'var(--bg-hover)'
                  }}
                  onMouseLeave={e => {
                    if (tab !== item.id) (e.currentTarget as HTMLButtonElement).style.background = 'transparent'
                  }}
                >
                  <span style={{ opacity: tab === item.id ? 1 : 0.5, display: 'flex', flexShrink: 0 }}>
                    {ICONS[item.id]}
                  </span>
                  {item.label}
                </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', maxWidth: 560 }}>
          ...
          <div className="card">
            <div className="card-header">
              <div className="card-title">Ad Soyad</div>
              <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-tertiary)' }}>Profilinizde görünen isim</div>
            </div>
            <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center' }}>
              <input
                ...
                style={{ flex: 1 }}
              />
              <button className="btn btn-primary" onClick={handleSaveProfile} disabled={profileLoading} style={{ whiteSpace: 'nowrap' }}>
                {profileLoading ? 'Kaydediliyor...' : 'Kaydet'}
              </button>
            </div>
            {profileMsg && (
              <div style={{
                marginTop: 'var(--space-3)', padding: '8px 12px', borderRadius: 6, fontSize: 13,
                background: profileMsg.startsWith('✅') ? 'var(--success-soft)' : 'var(--danger-soft)',
                color: profileMsg.startsWith('✅') ? 'var(--success)' : 'var(--danger)',
              }}>{profileMsg}</div>
            )}
          </div>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-tertiary)' }}>Giriş için kullanılan adres</div>
            </div>
            <input className="form-input" value={user?.email || ''} disabled style={{ opacity: 0.55, cursor: 'not-allowed' }} />
            <p style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 'var(--space-2)' }}>E-posta adresi değiştirilemez.</p>
          </div>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-tertiary)' }}>Arayüz dili</div>
            </div>
            <div style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'center' }}>
              <select className="form-input" value={selectedLang} onChange={e => setSelectedLang(e.target.value)} style={{ flex: 1 }}>
                {LANGUAGES.map(l => (
                  <option key={l.code} value={l.code}>{l.flag} {l.label}</option>
                ))}
              </select>
              <button className="btn btn-primary" onClick={handleSaveLanguage} style={{ whiteSpace: 'nowrap' }}>
                Kaydet {langMsg}
              </button>
            </div>
          </div>
          ...
          <div className="card" style={{ borderColor: 'var(--danger-soft)' }}>
            <div className="card-header">
              <div className="card-title" style={{ color: 'var(--danger)' }}>Oturumu Kapat</div>
            </div>
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-secondary)', marginBottom: 'var(--space-4)' }}>
              Tüm cihazlardaki aktif oturumunuz sonlandırılır.
            </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
              <div>
                ...
                <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-secondary)', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                  Şu an: {theme === 'dark' ? <><MoonIcon size={13} /> Koyu Tema</> : <><AppearanceIcon size={13} /> Açık Tema</>}
                </div>
              </div>
              <button className="btn btn-secondary" style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }} onClick={toggleTheme}>
                {theme === 'dark' ? <><AppearanceIcon size={13} /> Açık Temaya Geç</> : <><MoonIcon size={13} /> Koyu Temaya Geç</>}
              </button>
            </div>
          ...
          <div className="card" style={{ maxWidth: 700 }}>
            ...
            {backupLoading ? (
              ...
              <div className="empty-state" style={{ padding: 'var(--space-8)' }}>
                <div className="empty-state-icon"><BackupIcon size={48} /></div>
      ...
      {tab === 'integrations' && (() => {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
            ...
            {/* Google Card */}
            <ProviderCard
              icon={<svg width="28" height="28" viewBox="0 0 28 28"><circle cx="14" cy="14" r="12" fill="#4285F4"/><text x="14" y="19" textAnchor="middle" fill="white" fontSize="14" fontWeight="700">G</text></svg>} title="Google Takvim" color="#22c55e" accent="#4285F4"
            ...
            {/* Microsoft Card */}
            <ProviderCard
              icon={<svg width="28" height="28" viewBox="0 0 28 28"><circle cx="14" cy="14" r="12" fill="#0078D4"/><text x="14" y="19" textAnchor="middle" fill="white" fontSize="14" fontWeight="700">M</text></svg>} title="Microsoft Outlook Takvim" color="#0078D4" accent="#0078D4"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              ...
              {!updateInfo ? (
                <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'flex-end' }}>
                  ...
                  >
                    {updateChecking ? 'Kontrol ediliyor...' : <><SearchIcon size={13} /> Güncelleme Kontrol Et</>}
                  </button>
                </div>
              ) : updateInfo.has_update ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center' }}>
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setUpdateSuccess('Güncelleme başlatıldı. Sistem birkaç dakika içinde yeniden başlayacak.')
                          setUpdateLogsVisible(true)
                          // Poll logs after 10s to show progress/errors
                          setTimeout(async () => {
                            try {
                              const r = await updatesApi.logs()
                              setUpdateLogs(r.data?.logs || [])
                            } catch { /* ignore */ }
                          }, 10000)
                          setTimeout(() => {
                            setUpdateInfo(null)
                    ...
                    >
                      {updateTriggering ? '⏳ Başlatılıyor...' : <><RefreshIcon size={13} /> Şimdi Güncelle</>}
                    </button>
              ...
              {updateLogsVisible && (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 12, fontWeight: 600 }}>Güncelleme Geçmişi</span>
                    <button className="btn btn-ghost btn-sm" style={{ fontSize: 11 }} onClick={async () => {
                      try { const r = await updatesApi.logs(); setUpdateLogs(r.data?.logs || []) } catch { /* ignore */ }
                    }}>Yenile</button>
                    <button className="btn btn-ghost btn-sm" style={{ fontSize: 11 }} onClick={() => setUpdateLogsVisible(false)}>Kapat</button>
                  </div>
                  {updateLogs.length === 0 ? (
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Henüz kayıt yok. Birkaç saniye sonra yenileyin.</div>
                  ) : updateLogs.slice(0, 5).map((l: any) => (
                    <div key={l.id} style={{ fontSize: 11, background: 'var(--bg-elevated)', borderRadius: 6, padding: '6px 10px', marginBottom: 4 }}>
                      <span style={{ fontWeight: 600, color: l.status === 'success' ? 'var(--income)' : l.status === 'failed' ? 'var(--expense)' : 'var(--text-secondary)' }}>
                        [{l.status}]
                      </span>{' '}
                      v{l.version}
                      {l.output && <div style={{ color: 'var(--text-secondary)', marginTop: 2 }}>{l.output}</div>}
                      {l.error && <div style={{ color: 'var(--expense)', marginTop: 2 }}>{l.error}</div>}
                    </div>
                  ))}
                </div>
              )}
