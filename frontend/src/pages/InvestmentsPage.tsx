...
import { InvestmentIcon, UsersIcon, EditIcon, CloseIcon } from '../components/Icons'
    ...
    <div>
      ...
      {!selectedPortfolio && (
        loading ? (
          ...
          <div className="empty-state">
            <div className="empty-state-icon"><InvestmentIcon size={48} /></div>
                  ...
                  {p.group_names && p.group_names.length > 0 && (
                    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 'var(--space-2)' }}>
                      {p.group_names.map((g: string, i: number) => (
                        <span key={i} className="badge" style={{ background: 'var(--bg-elevated)', color: 'var(--text-secondary)', fontSize: 11, display: 'inline-flex', alignItems: 'center', gap: 3 }}><UsersIcon size={10} /> {g}</span>
                      ))}
                    </div>
                  )}
      ...
      <Modal open={showPortModal} onClose={() => setShowPortModal(false)}>
        <div className="modal-header">
          ...
          <button className="modal-close" onClick={() => setShowPortModal(false)}><CloseIcon size={14} /></button>
        </div>
      ...
      <Modal open={showTxModal} onClose={() => { setShowTxModal(false); setEditingTx(null) }}>
        <div className="modal-header">
          ...
          <button className="modal-close" onClick={() => { setShowTxModal(false); setEditingTx(null) }}><CloseIcon size={14} /></button>
        </div>
