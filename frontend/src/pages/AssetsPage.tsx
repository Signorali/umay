...
import { ASSET_TYPE_ICONS, AssetsIcon, MarketIcon, CloseIcon, SecurityAssetIcon, EditIcon } from '../components/Icons'
  ...
  return (
    <div>
      ...
      <div className="page-header">
        ...
        {canCreate && (
          <button className="btn btn-primary" onClick={() => { setEditingId(null); setForm({ ...EMPTY_FORM }); setFormError(''); setShowModal(true) }}>
            + Yeni Varlık
          </button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap', marginBottom: 'var(--space-4)' }}>
        {TYPES.map(typ => (
          ...
          >
            {typ === 'ALL' ? 'Tümü' : <><span style={{ display: 'inline-flex', verticalAlign: 'middle', marginRight: 4 }}>{ASSET_TYPE_ICONS[typ]}</span>{TYPE_LABELS[typ] || typ}</>}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <div className="empty-state-icon"><AssetsIcon size={48} /></div>
          ...
          {filtered.map((a: any) => {
            ...
            return (
              <div key={a.id} className="card" style={{ position: 'relative', cursor: 'pointer' }}
                ...
                {group && (
                  <span className="badge badge-neutral" style={{ position: 'absolute', top: 6, left: 6, fontSize: 9, padding: '2px 4px' }}>
                    {group.name}
                  </span>
                )}
                {/* Action buttons */}
                <div style={{ position: 'absolute', top: 'var(--space-2)', right: 'var(--space-2)', display: 'flex', gap: 4, zIndex: 10 }}
                  ...
                  {canUpdate && (
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        setEditingId(a.id)
                        setForm({
                          name: a.name, asset_type: a.asset_type, status: a.status,
                          purchase_date: a.purchase_date, purchase_value: a.purchase_value,
                          current_value: a.current_value, currency: a.currency, fx_rate: a.fx_rate,
                          description: a.description || '', notes: a.notes || '',
                          source_account_ids: a.source_account_ids || [],
                          loan_ids: a.loan_ids || [],
                          group_id: a.group_id || ''
                        })
                        setFormError('')
                  ...
                  {canUpdate && (
                    <button
                      className="btn btn-ghost btn-sm"
                      style={{ padding: '2px 6px', fontSize: 12 }}
                      onClick={(e) => { e.stopPropagation(); setSelectedAsset(a); setValForm({ value: String(Number(a.current_value)), valuation_date: new Date().toISOString().slice(0, 10), notes: '' }); setShowValModal(true) }}
                      title="Değer güncelle"
                    ><SecurityAssetIcon size={13} /></button>
                  )}
                ...
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-3)', marginTop: 'var(--space-2)' }}>
                  <span style={{ display: 'flex', opacity: 0.8 }}>{ASSET_TYPE_ICONS[a.asset_type] || ASSET_TYPE_ICONS['OTHER']}</span>
        ...
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal" style={{ maxWidth: 560 }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title">{editingId ? 'Varlığı Düzenle' : 'Yeni Varlık'}</span>
                    ...
                    <div className="form-group">
                      <label className="form-label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button type="button" className="btn btn-ghost btn-sm" style={{ fontSize: 11, padding: '1px 6px' }}
                          ...
                          <MarketIcon size={13} /> Piyasadan al
                        </button>
                      </label>
        ...
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setShowValModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowValModal(false)}><CloseIcon size={14} /></button>
            </div>
      ...
      {showHistoryModal && historyAsset && (
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setShowHistoryModal(false)}>
          <div className="modal" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <span className="modal-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>{ASSET_TYPE_ICONS[historyAsset.asset_type] || ASSET_TYPE_ICONS['OTHER']} {historyAsset.name} — Fiyat Geçmişi</span>
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowHistoryModal(false)}><CloseIcon size={14} /></button>
            </div>
            <div className="modal-body">
              {histLoading ? (
                <div className="loading-state"><div className="spinner" /></div>
        ...
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setShowSellModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowSellModal(false)}><CloseIcon size={14} /></button>
            </div>
