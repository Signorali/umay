    ...
    <div>
      ...
      {cards.length === 0 ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-4)', overflowX: 'auto', paddingBottom: 'var(--space-4)', marginBottom: 'var(--space-4)' }}>
            {cards.map((card: any, idx: number) => {
              const isSelected = selectedCard?.id === card.id
              const gradient = CARD_GRADIENTS[idx % CARD_GRADIENTS.length]
              return (
                <div key={card.id} onClick={() => setSelectedCard(card)} style={{
                  minWidth: 280, borderRadius: 'var(--radius-lg)', overflow: 'hidden',
                  cursor: 'pointer', boxShadow: isSelected ? '0 0 0 3px var(--primary)' : 'var(--shadow-md)',
                  transition: 'box-shadow 0.2s', flexShrink: 0,
                }}>
                  <div style={{ background: gradient, color: 'white', padding: 'var(--space-4)', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', top: -20, right: -20, width: 100, height: 100, borderRadius: '50%', background: 'rgba(255,255,255,0.08)' }} />
                    <div style={{ fontSize: 'var(--font-size-xs)', opacity: 0.7, marginBottom: 'var(--space-2)' }}>💳 {card.bank_name}</div>
                    <div style={{ fontWeight: 700, fontSize: 'var(--font-size-md)', marginBottom: 'var(--space-3)' }}>{card.card_name}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--font-size-xs)' }}>
                      <div><div style={{ opacity: 0.6 }}>Limit</div><div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600 }}>{card.currency} {fmt(Number(card.credit_limit))}</div></div>
                      <div style={{ textAlign: 'right' }}><div style={{ opacity: 0.6 }}>Borç</div><div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600 }}>{card.currency} {fmt(Number(card.current_debt))}</div></div>
                    </div>
                  </div>
                  <div style={{ background: 'var(--bg-card)', padding: 'var(--space-2) var(--space-4)', borderBottom: '1px solid var(--border)', fontSize: 'var(--font-size-xs)', color: 'var(--text-tertiary)', display: 'flex', gap: 'var(--space-4)' }}>
                    <span>📄 Kesim: {card.statement_day}</span>
                    <span>⏰ Son: {card.due_day}</span>
                  </div>
                </div>
              )
            })}
          ...
          <div style={{ display: 'flex', gap: 'var(--space-1)', borderBottom: '1px solid var(--border)', marginBottom: 'var(--space-4)' }}>
            {[
              ...
              <button key={tab.key}
                ...
                style={{ borderRadius: 'var(--radius-sm) var(--radius-sm) 0 0', padding: 'var(--space-2) var(--space-5)', fontWeight: activeTab === tab.key ? 600 : 400, borderBottom: activeTab === tab.key ? '2px solid var(--primary)' : 'none' }}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
                ...
                <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                  <button className="btn btn-secondary btn-sm" onClick={() => setShowPurchaseTemplates(true)}>
                    Kayıtlı {purchaseTemplates.length > 0 && <span style={{ marginLeft: 4, background: 'var(--primary)', color: '#fff', borderRadius: 10, padding: '1px 6px', fontSize: 11 }}>{purchaseTemplates.length}</span>}
                  </button>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ display: 'flex', gap: 'var(--space-2)', flexShrink: 0 }}>
                          ...
                          {stmt.status === 'OPEN' && Number(stmt.new_spending) > 0 && (
                            ...
                            }}>✏️ Detaylandır</button>
                          )}
                          {stmt.status === 'OPEN' && (
                            <>
                              <button className="btn btn-secondary btn-sm" style={{ color: 'var(--warning)' }} onClick={() => handleFinalize(stmt)}>🔒 Kesinleştir</button>
                              <button className="btn btn-ghost btn-sm" style={{ color: 'var(--expense)' }} onClick={() => handleDeleteStatement(stmt)}>🗑️ Sil</button>
                            </>
                          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', maxWidth: 600 }}>
              ...
              <div className="card" style={{ padding: 'var(--space-5)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
                  ...
                  <button className="btn btn-secondary btn-sm" onClick={openEditCard}>✏️ Düzenle</button>
                </div>
              ...
              <div className="card" style={{ padding: 'var(--space-5)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
                  <h3 style={{ fontWeight: 600, fontSize: 'var(--font-size-md)', margin: 0 }}>🔒 Hassas Bilgiler</h3>
                  <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                    <button className="btn btn-secondary btn-sm" onClick={() => { setSensitivePassword(''); setSensitiveCardNumber(''); setSensitiveCvv(''); setSensitiveError(''); setShowSensitiveSaveModal(true) }}>✏️ Güncelle</button>
                    {!sensitiveData && (
                      <button className="btn btn-primary btn-sm" onClick={() => { setSensitivePassword(''); setSensitiveError(''); setShowSensitiveRevealModal(true) }}>👁 Görüntüle</button>
                    )}
                    {sensitiveData && (
                      <button className="btn btn-ghost btn-sm" onClick={() => setSensitiveData(null)}>🔒 Gizle</button>
                    )}
                  </div>
                </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
                    <div>
                      ...
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--font-size-md)', fontWeight: 600, letterSpacing: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
                        {sensitiveData.card_number
                          ...
                          : '—'}
                        {sensitiveData.card_number && (
                          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowCardNumber(v => !v)} style={{ fontSize: 14 }}>{showCardNumber ? '🙈' : '👁'}</button>
                        )}
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {sensitiveData.cvv && (
                          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowCvv(v => !v)} style={{ fontSize: 14 }}>{showCvv ? '🙈' : '👁'}</button>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <Modal open={showEditCardModal} onClose={() => setShowEditCardModal(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowEditCardModal(false)}>✕</button>
        </div>
      ...
      <Modal open={showSensitiveSaveModal} onClose={() => setShowSensitiveSaveModal(false)}>
        <div className="modal-header">
          <span className="modal-title">🔒 Hassas Kart Bilgilerini Kaydet</span>
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowSensitiveSaveModal(false)}>✕</button>
        </div>
        <form onSubmit={handleSensitiveSave}>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-primary" disabled={sensitiveLoading}>{sensitiveLoading ? <span className="spinner spinner-sm" /> : '🔒 Şifreli Kaydet'}</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal open={showSensitiveRevealModal} onClose={() => setShowSensitiveRevealModal(false)}>
        <div className="modal-header">
          <span className="modal-title">🔐 Kart Bilgilerini Görüntüle</span>
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowSensitiveRevealModal(false)}>✕</button>
        </div>
        <form onSubmit={handleSensitiveReveal}>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-primary" disabled={sensitiveLoading}>{sensitiveLoading ? <span className="spinner spinner-sm" /> : '👁 Görüntüle'}</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal open={showCardModal} onClose={() => setShowCardModal(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowCardModal(false)}>✕</button>
        </div>
      ...
      <Modal open={showPurchaseModal} onClose={() => setShowPurchaseModal(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowPurchaseModal(false)}>✕</button>
        </div>
      ...
      <Modal open={showStatementModal} onClose={() => setShowStatementModal(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowStatementModal(false)}>✕</button>
        </div>
      ...
      <Modal open={showDetailModal} onClose={() => setShowDetailModal(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowDetailModal(false)}>✕</button>
        </div>
        <div className="modal-body">
          {error && <div className="alert alert-danger">{error}</div>}
          {detailTarget && (
      ...
      <Modal open={showPurchaseTemplates} onClose={() => setShowPurchaseTemplates(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowPurchaseTemplates(false)}>✕</button>
        </div>
        <div className="modal-body">
          {purchaseTemplates.length === 0 ? (
      ...
      <Modal open={showCancelModal} onClose={() => setShowCancelModal(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowCancelModal(false)}>✕</button>
        </div>
        <div className="modal-body">
          {error && <div className="alert alert-danger">{error}</div>}
          {cancelTarget && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <label style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-3)', padding: 'var(--space-3)', border: cancelScenario === 'A' ? '2px solid var(--primary)' : '1px solid var(--border)', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }} onClick={() => setCancelScenario('A')}>
            ...
            <label style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-3)', padding: 'var(--space-3)', border: cancelScenario === 'B' ? '2px solid var(--primary)' : '1px solid var(--border)', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }} onClick={() => setCancelScenario('B')}>
      ...
      <Modal open={showStatementView} onClose={() => setShowStatementView(false)}>
        <div className="modal-header">
          ...
          <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowStatementView(false)}>✕</button>
        </div>
        <div className="modal-body">
          {statementDetail && (
            <>
