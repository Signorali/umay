...
import { GroupsIcon, CloseIcon, EditIcon } from '../components/Icons'
...
function EditGroupModal({ group, onClose, onSaved }: { group: Group; onClose: () => void; onSaved: () => void }) {
  ...
  const submit = async () => {
    ...
    setSaving(true)
    try {
      await groupsApi.update(group.id, { name: form.name.trim(), description: form.description.trim() || null })
      onSaved()
    } catch (e: any) {
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-header">
          <span className="modal-title">Grubu Düzenle</span>
          <button className="btn btn-ghost btn-icon btn-sm" onClick={onClose}><CloseIcon size={14} /></button>
        </div>
        <div className="modal-body">
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <div className="empty-state-icon"><GroupsIcon size={48} /></div>
                  ...
                  <tr key={g.id}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                        ...
                        <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setEditingGroup(g)} title="Düzenle">
                          <EditIcon size={13} />
                        </button>
