      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          <button className="btn btn-secondary" onClick={() => setShowTemplates(true)}>
            Kayıtlı İşlemler {templates.length > 0 && <span style={{ marginLeft: 4, background: 'var(--accent)', color: '#fff', borderRadius: 10, padding: '1px 7px', fontSize: 'var(--font-size-xs)' }}>{templates.length}</span>}
          </button>
        ...
        <div className="modal-backdrop" onClick={() => setShowTemplates(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setShowTemplates(false)}><CloseIcon size={14} /></button>
            </div>
            <div className="modal-body">
              {templates.length === 0 ? (
                  ...
                  {templates.map((tmpl: any) => (
                    <div key={tmpl.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: 'var(--bg-elevated)' }}>
                      ...
                      <button className="btn btn-ghost btn-icon btn-sm" style={{ color: 'var(--expense)' }}
                        onClick={() => deleteTemplate(tmpl.id)} title="Şablonu sil"><CloseIcon size={12} /></button>
                    </div>
                  ))}
                </div>
              )}
