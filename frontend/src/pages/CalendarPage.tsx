...
import { CalendarIcon, ExportIcon } from '../components/Icons'
...
function IntegrationPanel() {
  ...
  const providers = [
    { key: 'google', label: 'Google Takvim', icon: <CalendarIcon size={22} />, color: '#4285F4' },
    { key: 'microsoft', label: 'Outlook Takvim', icon: <CalendarIcon size={22} />, color: '#0078D4' },
  ]
                ...
                <div>
                  ...
                  {isConnected ? (
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
                      {configured ? 'Bağlı değil' : 'Yapılandırma gerekli'}
                    </div>
                  )}
                </div>
              </div>
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
          <button className="btn btn-secondary" onClick={handleExportIcs} disabled={exporting} title="Outlook / Google Calendar için .ics indir">
            {exporting ? '⏳' : <ExportIcon size={13} />} Outlook'a Aktar
          </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedDate ? '1fr 300px' : '1fr', gap: 'var(--space-4)', alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: 'var(--space-4)' }}>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-4)', marginTop: 'var(--space-3)', fontSize: 11, color: 'var(--text-tertiary)', borderTop: '1px solid var(--border-subtle)', paddingTop: 'var(--space-3)' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#ef4444', display: 'inline-block' }} /> Gider / Ödeme</span>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e', display: 'inline-block' }} /> Gelir</span>
                    ...
                    <div key={item.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                        <span style={{ fontWeight: 600, fontSize: 'var(--font-size-sm)' }}>
                          <span style={{ width: 8, height: 8, borderRadius: '50%', background: income ? '#22c55e' : '#ef4444', display: 'inline-block', flexShrink: 0, marginRight: 4 }} />{item.title}
                        </span>
                        {url && (
