...
import { SecurityIcon, LicenseIcon, AlertIcon, RefreshIcon, DocumentsIcon } from '../components/Icons'
  ...
  return (
    <div>
        ...
        <div className="mfa-setup-card">
          ...
          {!status.mfa_enabled ? (
            <button className="btn btn-primary btn-lg" onClick={handleSetup} disabled={busy} style={{ width: '100%' }}>
              {busy ? <span className="spinner-sm spinner" /> : <><SecurityIcon size={15} /> Enable Two-Factor Authentication</>}
            </button>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
              <button className="btn btn-secondary" onClick={() => { setStep('regen'); setError(''); setSuccess('') }}>
                <RefreshIcon size={13} /> Regenerate Backup Codes
              </button>
      ...
      {step === 'backup-shown' && (
        <div className="mfa-setup-card">
          <div style={{ textAlign: 'center' }}>
            <div style={{ marginBottom: 'var(--space-2)', color: 'var(--text-secondary)' }}><LicenseIcon size={32} /></div>
            <div style={{ fontWeight: 700, fontSize: 'var(--font-size-lg)', marginBottom: 'var(--space-2)' }}>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            <button className="btn btn-secondary" style={{ flex: 1 }} onClick={copyBackupCodes}>
              <DocumentsIcon size={13} /> Copy codes
            </button>
      ...
      {step === 'disable' && (
        <div className="mfa-setup-card">
          <div style={{ textAlign: 'center' }}>
            <div style={{ marginBottom: 'var(--space-2)', color: 'var(--expense)' }}><AlertIcon size={32} /></div>
            <div style={{ fontWeight: 700, fontSize: 'var(--font-size-lg)', marginBottom: 'var(--space-2)' }}>
      ...
      {step === 'regen' && (
        <div className="mfa-setup-card">
          <div style={{ textAlign: 'center' }}>
            <div style={{ marginBottom: 'var(--space-2)', color: 'var(--text-secondary)' }}><RefreshIcon size={32} /></div>
            <div style={{ fontWeight: 700, fontSize: 'var(--font-size-lg)', marginBottom: 'var(--space-2)' }}>
              Regenerate Backup Codes
            </div>
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-secondary)' }}>
