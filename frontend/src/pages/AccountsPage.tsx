    ...
    <div className="stat-card" style={{ cursor: canSeeBalance ? 'pointer' : 'default', opacity: canSeeBalance ? 1 : 0.75 }} onClick={canSeeBalance ? onEdit : undefined}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-3)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
          <span style={{ fontSize: 20 }}>{TYPE_ICONS[account.account_type] || '🏦'}</span>
...
export function AccountsPage() {
  ...
  return (
    <div>
      ...
      {/* Tabs */}
      <div className="tabs" style={{ marginBottom: 'var(--space-6)', display: 'flex', gap: 'var(--space-1)', borderBottom: '1px solid var(--border)', overflowX: 'auto', paddingBottom: 2 }}>
        {accountTypes.map(type => {
          ...
          return (
            ...
            >
              <span style={{ marginRight: 8 }}>{TYPE_ICONS[type]}</span>
      ...
      {loading ? (
        ...
        <div className="empty-state" style={{ minHeight: '30vh' }}>
          <div className="empty-state-icon">{TYPE_ICONS[selectedTab]}</div>
