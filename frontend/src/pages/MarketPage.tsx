    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: 0 }}>
                      ...
                      {canView ? (
                        <button
                          ...
                          style={{
                            ...
                            fontSize: 15, opacity: isPinned ? 1 : 0.25,
                            transition: 'opacity 0.2s',
                            color: isPinned ? 'var(--accent)' : 'var(--text-tertiary)',
                          }}
                        >📌</button>
                      ) : (
                        <span style={{ fontSize: 15, opacity: isPinned ? 1 : 0.15, color: isPinned ? 'var(--accent)' : 'var(--text-tertiary)' }}>📌</span>
                      )}
                    </td>
                    <td>
                      ...
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                        ...
                        {canDelete && (
                          <button className="btn btn-ghost btn-sm" style={{ color: 'var(--danger)' }}
                            onClick={() => handleRemove(w.id)}>✕</button>
                        )}
                      </div>
                    </td>
                  </tr>
          ...
          <div style={{
            ...
            <span>📌 ile max {MAX_PINNED} sembolü panele sabitleyin</span>
      ...
      <Modal open={actionItem != null && (actionType === 'buy' || actionType === 'sell')} onClose={closeAction}>
        <div className="modal-header">
          ...
          <button className="modal-close" onClick={closeAction}>✕</button>
        </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {(['EXTERNAL', 'USER'] as const).map(t => (
                  <button key={t} type="button"
                    ...
                    {t === 'EXTERNAL' ? '🏢 Dış Kişi / Kurum' : '👤 Sistem Kullanıcısı'}
                  </button>
                ))}
              </div>
            </div>
      ...
      <Modal open={actionItem != null && actionType === 'plan'} onClose={closeAction}>
        <div className="modal-header">
          <div className="modal-title">📅 Planlı Ödeme — {actionItem?.symbol}</div>
          <button className="modal-close" onClick={closeAction}>✕</button>
        </div>
